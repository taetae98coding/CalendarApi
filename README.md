...
`solar` 는 항상 ISO-8601(proleptic Gregorian) 날짜입니다.
한국천문연구원은 그레고리력 도입(1582-10-15) 이전 구간을 율리우스력으로 제공하는데
(율리우스력에만 있는 1500-02-29 같은 날짜가 나옵니다),
달력 종류와 무관한 율리우스 적일(`solJd`)로 변환해 담기 때문입니다.
따라서 1582년 이전은 한국천문연구원이 표시하는 양력 날짜와 며칠 차이가 납니다.
원본 값은 `cache/kasi/lunar/` 에 그대로 남아 있습니다.

한국천문연구원은 월 단위 조회도 율리우스력 기준의 월로 내려주기 때문에
(1500년 2월을 요청하면 1500-02-10 ~ 1500-03-10 이 옵니다),
앞뒤 달을 함께 읽어 실제 양력 날짜로 다시 묶어 파일을 만듭니다.
1582년 그레고리력 개혁으로 같은 양력 날짜가 두 번 나오는 열흘은 하나만 남깁니다.
