...
import io.github.taetae98coding.calendar.cache.CacheProvider
import io.github.taetae98coding.calendar.cache.Granularity
...
/**
 * 고정 범위([Config.years]) 밖의 캐시와 배포 문서를 지운다.
 *
...
data object Pruner {
    ...
    private fun pruneCache() {
        ...
        Paths.cache.children().forEach { providerDirectory ->
    ...
    private fun SourceApi.expectedName(name: String): String? {
        ...
        if (year !in Config.years) return null
    ...
    private fun pruneDocs() {
        ...
        Paths.docs.children().forEach { apiDirectory ->
    ...
    private fun File.expectedDocName(): String? {
        ...
        return name.takeIf { year in Config.years }
    }
